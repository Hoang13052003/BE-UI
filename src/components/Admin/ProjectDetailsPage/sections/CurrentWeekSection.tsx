...
const CurrentWeekSection: React.FC<CurrentWeekSectionProps> = ({ data, type }) => {
  // Render timeLogs
  if (type === 'timelog') {
    return (
      <Card title={<Title level={4}>Current Week Time Logs</Title>} style={{ marginBottom: 24 }}>
        <Text type="secondary">{data.description}</Text>
        <div style={{ margin: '12px 0' }}>
          <Tag color="green">Total: {data.totalCount}</Tag>
        </div>
  ...
  // Render milestones
  if (type === 'milestone') {
    console.log('CurrentWeekSection - Milestones data:', data.milestones);
    return (
      <Card title={<Title level={4}>Current Week Milestones</Title>} style={{ marginBottom: 24 }}>
        <Text type="secondary">{data.description}</Text>
        <div style={{ margin: '12px 0' }}>
          <Tag color="green">Total: {data.totalCount}</Tag>
        </div>
