...
import { TreeNodeDto as ApiTreeNodeDto, AttachmentResponseDto } from '../../../types/Attachment'; // Đổi tên DTO từ API để tránh trùng
...
const mapApiNodeToDataNode = (apiNode: ApiTreeNodeDto): DataNode => ({
  ...
  icon: apiNode.type === 'directory' ? <FolderOutlined /> : <FileOutlined />,
...
const updateTreeDataRecursive = (list: DataNode[], key: React.Key, children: DataNode[]): DataNode[] => {
...
const AttachmentsTree: React.FC<AttachmentsTreeProps> = ({ projectId, projectName }) => {
  ...
  useEffect(() => {
    if (projectId) {
      const fetchRootNodes = async () => {
        ...
        setExpandedKeys([]);
        try {
          const rootApiNodes = await attachmentApi.getCurrentProjectTreeRoot(projectId);
          if (rootApiNodes.length === 0) {
            message.info(`No files or folders found at the root of project ${projectName || `ID ${projectId}`}.`);
          }
          setTreeData(rootApiNodes.map(mapApiNodeToDataNode));
        } catch (error: any) {
          console.error("Failed to load root attachments:", error);
          message.error(error.response?.data?.error || `Failed to load root files for project.`);
          setTreeData([{ title: 'Error loading root', key: 'root-error', isLeaf: true, icon: <FileOutlined />}]);
        } finally {
          setLoadingKeys([]);
        }
      };
  ...
  const onLoadData = useCallback((node: DataNode): Promise<void> => {
    const { key: nodePath, children, loaded, isLeaf } = node;

    return new Promise(async (resolve) => {
      if (children || loaded || isLeaf || !projectId) { // Đã có con, hoặc đã load, hoặc là file, hoặc không có projectId
        resolve();
        return;
      }
      ...
      try {
        ...
        const childrenApiNodes = await attachmentApi.getCurrentProjectTreeByPath(projectId, nodePath);
        const childrenDataNodes = childrenApiNodes.map(mapApiNodeToDataNode);

        setTreeData((origin) => updateTreeDataRecursive(origin, nodePath, childrenDataNodes));
        // Tự động mở rộng node vừa được load
        setExpandedKeys(prevKeys => prevKeys.includes(nodePath) ? prevKeys : [...prevKeys, nodePath]);

      } catch (error: any) {
        console.error(`Failed to load children for ${nodePath}:`, error);
        message.error(error.response?.data?.error || `Failed to load content for ${node.title}.`);
        // Cập nhật node với thông báo lỗi
        const errorNodeChild: DataNode[] = [{ title: 'Error loading content', key: `${nodePath}-error-${Date.now()}`, isLeaf: true, icon: <FileOutlined /> }];
        setTreeData((origin) => updateTreeDataRecursive(origin, nodePath, errorNodeChild));
      } finally {
  ...
  const handleFileView = async (attachmentNode: DataNode) => {
    ...
    message.loading({ content: `Preparing ${fileName}...`, key: `file-${attachmentId}` });
    try {
      const response = await attachmentApi.getPresignedUrl(attachmentId, "inline");
      message.success({ content: `Opening ${fileName}...`, key: `file-${attachmentId}`, duration: 2 });
      window.open(response.url, '_blank');
    } catch (error: any) {
      message.error({ content: error.response?.data?.error || `Failed to get URL for ${fileName}.`, key: `file-${attachmentId}`, duration: 3 });
  ...
  const renderTitle = (node: DataNode) => {
    if (loadingKeys.includes(node.key) && !node.loaded) {
      return (
        <>
          {node.icon} <Text>{node.title}</Text> <LoadingOutlined style={{ marginLeft: 8 }}/>
        </>
      );
    }
    return (
  ...
  if (loadingKeys.includes('root-loading') && treeData.length === 0) {
    return (
      <Card title={`Project Files ${projectName ? `- ${projectName}` : ''}`}>
        <div style={{textAlign: 'center', padding: '20px'}}><Spin tip="Loading project structure..." /></div>
      </Card>
    );
  }
    ...
    <Card title={`Project Files ${projectName ? `- ${projectName}` : ''}`}>
      {treeData.length > 0 ? (
        ...
        />
         ...
         !loadingKeys.includes('root-loading') && <Text type="secondary">No files or folders found for this project.</Text>
      )}
    </Card>
  );
};
