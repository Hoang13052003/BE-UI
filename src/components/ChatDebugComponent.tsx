...
const ChatDebugComponent: React.FC = () => {
  const { chatRooms, messages, activeChatRoom, selectChatRoom, loading } =
    useChat();
  ...
  useEffect(() => {
    ...
    if (currentRoomId !== lastActiveRoomId) {
      if (currentRoomId) {
        addLog(
          `🔄 State Change: Active room changed to "${activeChatRoom?.roomName}" (${currentRoomId})`
        );
      } else if (lastActiveRoomId) {
        addLog(
          `🔄 State Change: Active room cleared (was ${lastActiveRoomId})`
        );
      }
    ...
    if (currentMessageCount !== lastMessageCount) {
      addLog(
        `📊 State Change: Messages count changed from ${lastMessageCount} to ${currentMessageCount}`
      );
  ...
  const testAll = async () => {
    ...
    try {
      ...
      if (roomsResponse.length > 0) {
        ...
        addLog(
          `📋 API: First room "${firstRoom.roomName}" (ID: ${firstRoom.id})`
        );
        addLog(`🆔 API: Project ID: ${firstRoom.projectId || "N/A"}`);

        if (firstRoom.projectId) {
          const historyResponse = await getProjectChatHistory(
            firstRoom.projectId
          );
      ...
      addLog(
        `📊 Context: ${chatRooms.length} rooms, ${messages.length} messages`
      );
      ...
      if (chatRooms.length > 0 && roomsResponse.length > 0) {
        ...
        addLog(
          `🔍 IDs match: ${contextRoom.id === apiRoom.id ? "✅ Yes" : "❌ No"}`
        );
      }
      ...
      const messageElements = document.querySelectorAll(
        '[style*="marginBottom: 24"], .ant-list-item'
      );
      ...
      if (messages.length > 0) {
        ...
        addLog(
          `💬 First message: "${firstMessage.content}" by ${firstMessage.senderName}`
        );
      ...
      if (hasActiveRoom && hasMessages && hasUIMessages && notLoading) {
        ...
        if (chatRooms.length > 0 && !hasActiveRoom)
          addLog("  ⚠️ No room selected");
        if (hasActiveRoom && !hasMessages && !loading)
          addLog("  ❌ Room selected but no messages");
        if (hasMessages && !hasUIMessages)
          addLog("  ❌ Messages in context but not displayed");
    ...
    >
      ...
      >
        ...
        <p style={{ margin: "4px 0" }}>
          ...
          {activeChatRoom
            ? `${activeChatRoom.roomName} (ID: ${activeChatRoom.id})`
            : "None"}
        </p>
      </div>
      {/* Test Buttons */}
        ...
        <div style={{ marginTop: "16px" }}>
          ...
          {chatRooms.map((room, index) => (
            <div
              ...
              onClick={() => {
                addLog(
                  `🖱️ User Click: Selecting room "${room.roomName}" (${room.id})`
                );
                addLog(
                  `📋 Before Click: activeChatRoom = ${
              ...
              style={{
                padding: "12px",
                backgroundColor:
                  activeChatRoom?.id === room.id ? "#d4edda" : "#f8f9fa",
