...
export const useInlineEditMilestone = ({
  ...
  const [editedData, setEditedData] = useState<Record<number, Partial<BatchUpdateMilestoneItemDTO>>>({});
  const [batchSaving, setBatchSaving] = useState<boolean>(false);

  const handleInlineEdit = useCallback((milestoneId: number, field: keyof BatchUpdateMilestoneItemDTO, value: any) => {
    setEditedData(prev => ({
      ...prev,
      [milestoneId]: {
        ...prev[milestoneId],
    ...
    // Cập nhật milestones để UI phản ánh ngay
  ...
  const handleBatchSave = useCallback(async () => {
    const changesToSubmit: BatchUpdateMilestoneItemDTO[] = Object.values(editedData)
    ...
    try {
      ...
      message.success(`${changesToSubmit.length} milestone(s) updated successfully!`);
      setEditedData({});
      onRefreshData();
    } catch (err: any) {
      console.error('Error batch saving milestones:', err);
      
      // Handle specific error cases theo hướng dẫn BE
      if (err.response?.status === 400) {
        message.error('Validation error: Please check all fields or ensure this is a FIXED_PRICE project.');
      } else if (err.response?.status === 404) {
        message.error('Some milestones were not found. Please refresh and try again.');
  ...
  const handleMarkSelectedAsCompleted = useCallback(async (selectedRowKeys: React.Key[]) => {
    ...
    const updates: BatchUpdateMilestoneItemDTO[] = selectedRowKeys.map(key => ({
      id: Number(key),
    ...
    try {
      ...
      // Handle specific error cases theo hướng dẫn BE
      if (err.response?.status === 400) {
        message.error('Cannot mark as completed: Please ensure this is a FIXED_PRICE project.');
      } else if (err.response?.status === 404) {
        message.error('Some milestones were not found. Please refresh and try again.');
      } else if (err instanceof Error) {
        message.error(err.message);
