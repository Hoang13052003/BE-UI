...
interface UseAttachmentUploadProps {
  onUploadProgress?: (
    file: UploadFile | FolderFileItem,
    percent: number
  ) => void;
  onUploadSuccess?: (
    item: UploadFile | FolderFileItem,
...
export const useAttachmentUpload = (props?: UseAttachmentUploadProps) => {
  ...
  const uploadFilesIndividually = async (
    ...
    setIsUploading(true);
    message.info(
      `Uploading ${antdFileList.length} attachment(s) individually...`,
      2
    );

    console.log(
      "Uploading attachment(s) individually...: " + antdFileList.length
    );
    ...
    const uploadPromises = antdFileList.map(async (antdFile) => {
      if (antdFile.originFileObj) {
        ...
        try {
          const response = await uploadSingleAttachment(
            fileToUpload,
            feedbackId,
            projectUpdateId,
            logicalName
          ...
          console.log("data response add file: " + response);

          props?.onUploadSuccess?.(antdFile, response);
          return {
            status: "fulfilled",
            value: response,
            itemName: antdFile.name,
          } as const;
        } catch (error: any) {
    ...
    results.forEach((result) => {
      if (result.status === "fulfilled") {
        ...
        if (innerResult.status === "fulfilled" && innerResult.value) {
          ...
          failedUploads.push({
            itemName: innerResult.itemName,
            reason: innerResult.reason,
          });
        }
      } else {
        console.error(
          "Unexpected error in Promise.allSettled for individual file uploads:",
          result.reason
        );
      }
    });
    ...
    if (failedUploads.length > 0) {
      ...
      failedUploads.forEach((fail) =>
        console.error(`Upload failed for ${fail.itemName}: ${fail.reason}`)
      );
    }
    if (successfulUploads.length > 0 && failedUploads.length === 0) {
      message.success(
        `${successfulUploads.length} attachment(s) uploaded successfully.`,
        3
      );
    } else if (successfulUploads.length > 0 && failedUploads.length > 0) {
      message.warning(
        `Partially successful: ${successfulUploads.length} uploaded, ${failedUploads.length} failed.`,
        4
      );
    }
  ...
  const uploadFolderContents = async (
    ...
    setIsUploading(true);
    message.info(
      `Uploading folder contents (${itemsToUpload.length} file(s)) as a single batch...`,
      2
    );

    try {
      const response = await uploadFolderAttachments(
        itemsToUpload,
        projectUpdateId
      );
      ...
      const failedUploads: { itemName: string; reason: string }[] =
        response.uploadErrors.map((error) => ({
          itemName:
            error.logicalName ||
      ...
      if (failedUploads.length > 0) {
        message.error(
          `${failedUploads.length} file(s) from folder failed to upload.`,
          5
        );
        failedUploads.forEach((fail) =>
          console.error(
            `Folder upload failed for ${fail.itemName}: ${fail.reason}`
          )
        );
      }
      if (successfulUploads.length > 0 && failedUploads.length === 0) {
        message.success(
          `${successfulUploads.length} file(s) from folder uploaded successfully.`,
          3
        );
      } else if (successfulUploads.length > 0 && failedUploads.length > 0) {
        message.warning(
      ...
      const errorMsg =
        ...
        error.message ||
        "Failed to upload folder contents.";
