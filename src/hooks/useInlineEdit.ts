...
interface UseInlineEditProps {
  ...
  setCurrentPerformersMap: React.Dispatch<React.SetStateAction<Record<number, UserIdAndEmailResponse & { fullName?: string }>>>;
  onRefreshData: () => void;
}
...
export const useInlineEdit = ({
  ...
  const handleInlineEdit = useCallback((timelogId: number, field: keyof BatchUpdateItem, value: any) => {
    setEditedData(prev => ({
      ...prev,
      [timelogId]: {
        ...prev[timelogId],
    ...
    // Cập nhật timelogs để UI phản ánh ngay
    if (field === 'performerId') {
      const selectedUser = searchedUsers.find(u => u.id === value) || currentPerformersMap[timelogId];
      if (selectedUser) {
  ...
  const handleBatchSave = useCallback(async () => {
    const changesToSubmit: BatchUpdateItem[] = Object.values(editedData)
      .filter((item): item is BatchUpdateItem => item.id !== undefined && Object.keys(item).length > 1);
    ...
    try {
      ...
      message.success(`${changesToSubmit.length} time log(s) updated successfully!`);
      setEditedData({});
      onRefreshData();
    } catch (err) {
