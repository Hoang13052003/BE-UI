...
import { ArrowLeftOutlined, EditOutlined, CompressOutlined } from '@ant-design/icons';
...
const ProjectDetailPage: React.FC = () => {
  ...
  const fetchProjectData = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || "Failed to load project details. The project may not exist or an error occurred.");
    } finally {
      setLoading(false);
    }
  }, [projectId]);
  ...
  const onLayoutChange = (_layout: Layout[], layouts: any) => {
    ...
    localStorage.setItem(`project-layout-${projectId}`, JSON.stringify(layouts));
  };
  ...
  if (!project) {
    return (
      <div style={{ padding: '20px' }}>
        ...
        <Alert message="Project Not Found" description="The requested project could not be found." type="warning" showIcon />
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <Title level={2} style={{ color: theme === 'dark' ? '#fff' : undefined, marginBottom: '24px' }}>
        {project.name}
      </Title>
