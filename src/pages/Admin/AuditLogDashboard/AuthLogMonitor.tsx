...
const AuthLogMonitor: React.FC = () => {
  ...
  const columns: ColumnsType<AuditLog> = [
    { title: 'Timestamp', dataIndex: 'timestamp', render: (ts) => new Date(ts).toLocaleString() },    { title: 'Username', dataIndex: 'username' },
    { title: 'Action', dataIndex: 'actionType' },
    { 
      title: 'Status', 
      dataIndex: 'success', 
      render: (s) => s ? <Tag color="success">Success</Tag> : <Tag color="error">Failure</Tag>,
      filters: [
  ...
  return (
    <Layout style={{ minHeight: '100vh', backgroundColor: '#f0f2f5' }}>
      <Header style={{ backgroundColor: '#fff', padding: '0 24px' }}>
        <Title level={3} style={{ margin: '16px 0' }}>Authentication Log Monitor</Title>
      </Header>
      <Content style={{ padding: '24px' }}>
        {error && <Alert message="Error" description={error} type="error" showIcon closable style={{ marginBottom: '24px' }} />}
        
        <Space direction="vertical" size="large" style={{ width: '100%' }}>
          <Card loading={loading}>
            <Meta
              title="Statistics"
              description={`Total Auth Logs: ${stats?.total_auth_logs ?? 'N/A'} | Today's Logs: ${stats?.today_logs ?? 'N/A'}`}
            />
          </Card>
